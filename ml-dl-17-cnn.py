...
class AutoEncoder_classifier(nn.Module):
    def __init__(self, in_dim=8,  out_dim=8):
        ...
        self.encode = nn.Sequential(
            ...
            nn.Dropout(p=0.1),
            nn.Linear(in_features=400, out_features=1600),
            nn.Dropout(p=0.1),
        )
        ...
        self.classifier = nn.Sequential(
            ...
            nn.ReLU(),  # �����
            nn.Dropout(p=0.2),
            nn.MaxPool2d(kernel_size=(5, 5), stride=3),
            nn.Dropout(p=0.2),
            nn.Flatten(start_dim=1, end_dim=-1),
            nn.Linear(in_features=3872, out_features=2048),
            nn.Dropout(p=0.1),
            nn.Linear(in_features=2048, out_features=900),
            nn.Dropout(p=0.1),
            nn.Linear(in_features=900, out_features=in_dim),
            nn.Dropout(p=0.1),
    ...
    def forward(self, x):
        ...
        encoder_out = torch.reshape(encoder_out,(-1,1,40,40))
        # torch.Size([100, 400])
        # torch.Size([100, 1, 20, 20])

        decoder_out = self.classifier(encoder_out)
        return  decoder_out
...
class AutoEncoder_classifier(nn.Module):
    def __init__(self, in_dim=8,  out_dim=8):
        ...
        self.classifier = nn.Sequential(
            nn.Conv2d(1, 64, kernel_size=(5, 5), stride=1),
            nn.ReLU(),  # �����
            nn.Dropout(p=0.1),
            nn.Conv2d(64, 128, kernel_size=(5, 5), stride=1),
            nn.ReLU(),  # �����
            nn.Dropout(p=0.1),
            nn.Conv2d(128, 32, kernel_size=(5, 5), stride=1),
            nn.ReLU(),  # �����
            nn.Dropout(p=0.1),
            nn.MaxPool2d(kernel_size=(5, 5), stride=3),
            nn.Dropout(p=0.1),
            ...
            nn.Dropout(p=0.1),
            nn.Linear(in_features=100, out_features=in_dim),
            nn.Dropout(p=0.1),
            nn.Linear(in_features=in_dim, out_features=out_dim)
        )
...
if torch.cuda.is_available():
    autoEncoder.cuda()  # ע:��ģ�ͷŵ�GPU��,��˺�����������ݱ���Ҳ��GPU��
...
torch_dataset = Data.TensorDataset(torch.from_numpy(train_x_tr.values).to(torch.float32), torch.from_numpy(train_y_tr.values).to(torch.float32))
